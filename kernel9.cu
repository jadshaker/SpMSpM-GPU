...
__global__ void spmspm_kernel_9(COOMatrix *cooMatrix1,
                                        ...
                                        const unsigned int numNonzeros2)
{
    ...
    for (unsigned int rowOffset = 0; rowOffset < ROW_TILE_SIZE; rowOffset++) {
        ...
        if (nnz > 0) {
            if (tid == 0) {
                outputIdx = atomicAdd(&cooMatrix3->numNonzeros, nnz);
            }
            ...
            // Reset nnz counter for next row
            if (tid == 0) {
                nnz = 0;
            }
        }
                ...
                unsigned int numNonzeros2)
{
    ...
    spmspm_kernel_optimized<<<grid, block, sharedMemSize>>>(
        cooMatrix1,
        csrMatrix1,
        cscMatrix1,
        cooMatrix2,
